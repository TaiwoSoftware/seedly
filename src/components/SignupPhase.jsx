  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const checkResponse = await fetch(
        `https://simon-4636.restdb.io/rest/datauser?q={"phoneNumber":"${phonenumber}"}`,
        {
          method: "GET",
          headers: {
      ...
      const response = await fetch(
        "https://simon-4636.restdb.io/rest/datauser",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
        ...
        <div className="w-full max-w-md">
          ...
          <form className="max-w-full box-border p-2" onSubmit={handleSubmit}>
            ...
            <FormInput
              label={"Phone number"}
              inputStyles={
                "border bg-[#F2F2F2] outline-none rounded w-full p-2"
              }
              connect={"number"}
